...
def getTrainBatch_train():
    ...
    for i in range(batch_size):
        if (i % 2 == 0):
            ...
            num = randint(12500, 21499)
            labels.append([0, 1])
        arr[i] = training_data[num]
    return arr, labels

def getTrainBatch_test():
    labels = []
    arr = np.zeros([batch_size, seq_length])
    for i in range(batch_size):
        if (i % 2 == 0):
            num = randint(9001, 12499)
            labels.append([1, 0])
        else:
...
logdir = "tensorboard/" + datetime.datetime.now().strftime(
    "%Y%m%d-%H%M%S") + "/"
logdir_test = "tensorboard/test/"

writer = tf.summary.FileWriter(logdir, sess.graph)
writer_test = tf.summary.FileWriter(logdir_test, sess.graph)

for i in range(iterations):
    batch_data, batch_labels = getTrainBatch_train()
...
for i in range(test_iterations):
    batch_data, batch_labels = getTrainBatch_test()
    loss_value, accuracy_value, summary = sess.run(
        [loss, accuracy, summary_op],
        {input_data: batch_data,
         labels: batch_labels})
